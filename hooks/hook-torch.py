...
if is_module_satisfies("PyInstaller >= 6.0"):
    ...
    def _torch_filter(name):
        return not any(x in name for x in ('tensorboard', 'distributed', 'testing', 'caffe2'))

    hiddenimports = collect_submodules("torch", filter=_torch_filter, on_error="ignore")
